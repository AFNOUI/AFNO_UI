"use client";

import { useState } from "react";
import { Globe } from "lucide-react";
import {
  Combobox,
  ComboboxList,
  ComboboxItem,
  ComboboxEmpty,
  ComboboxInput,
  ComboboxGroup,
  ComboboxTrigger,
  ComboboxContent,
} from "@/components/ui/combobox";
import { ComponentInstall } from "@/components/lab/ComponentInstall";
import { code, data } from "@/registry/combobox/combobox-country";

export function ComboboxCountry() {
  const [country, setCountry] = useState("");

  return (
    <ComponentInstall category="combobox" variant="combobox-country" title="Country Selector" code={`const [country, setCountry] = useState("");

const data = ${JSON.stringify(data, null, 2)};

<Combobox value={country} onValueChange={setCountry}>
  <ComboboxTrigger className="w-[240px]">
    <div className="flex items-center">
      <Globe className="me-2 h-4 w-4 opacity-50" />
    {country ? (
      <span className="flex items-center gap-2">
          {data.find(c => c.value === country)?.flag}
          {data.find(c => c.value === country)?.label}
        </span>
      ) : "Select country..."}
    </div>
  </ComboboxTrigger>
  <ComboboxContent className="w-[240px]">
    <ComboboxInput placeholder="Search country..." />
    <ComboboxList>
      <ComboboxEmpty>No country found.</ComboboxEmpty>
      <ComboboxGroup>
        {data.map((c) => (
          <ComboboxItem key={c.value} value={c.value}>
            <span className="me-2">{c.flag}</span>
            {c.label}
          </ComboboxItem>
        ))}
      </ComboboxGroup>
    </ComboboxList>
  </ComboboxContent>
</Combobox>`} fullCode={code}>
      <Combobox value={country} onValueChange={setCountry}>
        <ComboboxTrigger className="w-[240px]">
          <div className="flex items-center">
            <Globe className="me-2 h-4 w-4 opacity-50" />
            {country ? (
              <span className="flex items-center gap-2">
                <span>{data.find(c => c.value === country)?.flag}</span>
                <span>{data.find(c => c.value === country)?.label}</span>
              </span>
            ) : "Select country..."}
          </div>
        </ComboboxTrigger>
        <ComboboxContent className="w-[240px]">
          <ComboboxInput placeholder="Search country..." />
          <ComboboxList>
            <ComboboxEmpty>No country found.</ComboboxEmpty>
            <ComboboxGroup>
              {data.map((c) => (
                <ComboboxItem key={c.value} value={c.value}>
                  <div className="flex items-center gap-2">
                    <span>{c.flag}</span>
                    <span>{c.label}</span>
                  </div>
                </ComboboxItem>
              ))}
            </ComboboxGroup>
          </ComboboxList>
        </ComboboxContent>
      </Combobox>
    </ComponentInstall>
  );
}