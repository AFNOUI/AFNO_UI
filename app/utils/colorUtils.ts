...
/**
 * Value for `:root` / `.dark` custom properties.
 * When tokens are consumed as `hsl(var(--name))` (see `app/globals.css` @theme), use raw **H S% L%** channels — no `hsl()` wrapper.
 */
export function formatCssVariableColorValue(hslChannels: string, format: ColorFormat): string {
  const c = hslChannels.trim();
  if (format === "hsl") return c;
